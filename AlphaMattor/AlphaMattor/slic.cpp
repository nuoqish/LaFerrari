	...
	const cv::Scalar&		color )
{
    ...
    for(int i = 0; i < _kseedsl.size(); i++) {
        int index = int(_kseedsy[i]) * width + int(_kseedsx[i]);
        ubuff[index] = 0;
        ubuff[index] |= (int)color.val[0] << 16; // r
        ubuff[index] |= (int)color.val[1] << 8; // g
        ubuff[index] |= (int)color.val[2];
    }
...
//                    _kseedsl.erase(_kseedsl.begin() + label);
//                    _kseedsa.erase(_kseedsa.begin() + label);
//                    _kseedsb.erase(_kseedsb.begin() + label);
//                    _kseedsx.erase(_kseedsx.begin() + label);
//                    _kseedsy.erase(_kseedsy.begin() + label);
                                  ...
                                  const double&				m)//weight given to spatial distance
{
    ...
    vector<double> edgemag(0);
    if(perturbseeds) DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
    GetLABXYSeeds_ForGivenK(_kseedsl, _kseedsa, _kseedsb, _kseedsx, _kseedsy, K, perturbseeds, edgemag);
    ...
    PerformSuperpixelSegmentation_VariableSandM(_kseedsl,_kseedsa,_kseedsb,_kseedsx,_kseedsy,klabels,STEP,10);
    numlabels = _kseedsl.size();
    
    
    int* nlabels = new int[sz];
    EnforceLabelConnectivity2(klabels, m_width, m_height, nlabels, numlabels, K);
    
    FILE *file = fopen("/Users/longyan/Desktop/WorkSpace/ImageSpace/AlphaMatting/Testset/1-seeds.txt", "wb");
    for (int i = 0; i < numlabels; i++) {
        fprintf(file, "%f %f %f %f %f\n", _kseedsl[i], _kseedsa[i], _kseedsb[i], _kseedsx[i], _kseedsy[i]);
    }
    fclose(file);
...
cv::Mat SLIC::GetImgWithContours(cv::Scalar color)
{
    ...
    return Mat(m_height, m_width, CV_8UC1);
}
...
vector<double>& SLIC::kseedsl() {
    return _kseedsl;
}

vector<double>& SLIC::kseedsa() {
    return _kseedsa;
}

vector<double>& SLIC::kseedsb() {
    return _kseedsb;
}
vector<double>& SLIC::kseedsx() {
    return _kseedsx;
}
vector<double>& SLIC::kseedsy() {
    return _kseedsy;
}
